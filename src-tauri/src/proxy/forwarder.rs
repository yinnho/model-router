...
impl RequestForwarder {
    ...
    async fn forward(
        ...
        } else if is_full_url || codex_chat_base_is_full_endpoint || codex_anthropic_base_is_full_endpoint {
        ...
        let request_body = if codex_responses_to_chat {
            ...
            super::providers::transform_codex_anthropic::responses_to_anthropic_messages(mapped_body)?
        ...
        let force_identity_encoding =
            needs_transform || codex_responses_to_chat || codex_responses_to_anthropic || request_is_streaming;
        ...
        // 获取认证头（提前准备，用于内联替换）
        // Anthropic wire mode 的 Codex provider 需要用 x-api-key 替代 Authorization: Bearer
        let use_anthropic_auth = codex_responses_to_anthropic;
        ...
        // Anthropic wire mode: 将 Authorization: Bearer 转为 x-api-key + anthropic-version
        if use_anthropic_auth {
            let mut api_key = None;
            auth_headers.retain(|(name, value)| {
                if name == "authorization" {
                    // 提取 Bearer token 中的 key
                    if let Ok(s) = value.to_str() {
                        api_key = s.strip_prefix("Bearer ").or_else(|| s.strip_prefix("bearer ")).map(|k| k.trim().to_string());
                    }
                    false // 移除 Authorization header
                } else {
                    true
                }
            });
