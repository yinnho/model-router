...
/// 处理 /v1/models 请求（Codex CLI 模型发现）
pub async fn handle_codex_models(
    ...
    let model = provider
        .settings_config
        .get("model")
        .or_else(|| provider.settings_config.get("config").and_then(|c| c.get("model")))
        .and_then(|v| v.as_str())
        .unwrap_or("unknown");
...
/// Anthropic Messages → Codex Responses 转换处理
///
/// 将上游 Anthropic Messages 的 SSE 流或 JSON 响应转换为 Codex Responses API 格式
async fn handle_codex_anthropic_to_responses_transform(
    ...
    // Non-streaming: Anthropic Messages JSON → Responses JSON
    let _connection_guard = connection_guard;
    let body_timeout =
        if ctx.app_config.auto_failover_enabled && ctx.app_config.non_streaming_timeout > 0 {
            std::time::Duration::from_secs(ctx.app_config.non_streaming_timeout as u64)
    ...
    let responses_response = transform_codex_anthropic::anthropic_messages_to_response(anthropic_response)
        .map_err(|e| {
            log::error!("[Codex/Anthropic] Anthropic → Responses 响应转换失败: {e}");
            e
        })?;
    ...
    let response_body = serde_json::to_vec(&responses_response).map_err(|e| {
        log::error!("[Codex/Anthropic] 序列化 Responses 响应失败: {e}");
        ProxyError::TransformError(format!("Failed to serialize responses response: {e}"))
    })?;
    ...
    builder
        .body(axum::body::Body::from(response_body))
        .map_err(|e| {
            log::error!("[Codex/Anthropic] 构建 Responses 响应失败: {e}");
            ProxyError::Internal(format!("Failed to build response: {e}"))
        })
}
