...
/// Whether this Codex provider's real upstream should be called through
/// Anthropic Messages API, even if the local Codex client is talking to CC
/// Switch through the Responses API.
pub fn codex_provider_uses_anthropic_messages(provider: &Provider) -> bool {
    if let Some(api_format) = provider
        .meta
        .as_ref()
        .and_then(|meta| meta.api_format.as_deref())
    ...
    if let Some(wire_mode) = provider
        .settings_config
        .get("wire_mode")
        .and_then(|v| v.as_str())
    {
        return is_anthropic_wire_api(wire_mode);
    }
...
pub fn should_convert_codex_responses_to_anthropic(provider: &Provider, endpoint: &str) -> bool {
    ...
    matches!(
        path,
        "/responses" | "/v1/responses" | "/responses/compact" | "/v1/responses/compact"
    ) && codex_provider_uses_anthropic_messages(provider)
}
