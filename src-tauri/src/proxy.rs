...
fn resolve_tag_from_model(model: &str) -> Option<String> {
    ...
    if lower.contains("opus") {
        ...
        // Zhipu GLM models (e.g. glm-5.1)
        Some("sonnet".to_string())
    } else if lower.contains("deepseek") {
        // DeepSeek models
        Some("haiku".to_string())
    } else if lower.contains("k2") || lower.contains("kimi") {
...
pub async fn handle_anthropic_messages(
    ...
    handle_proxy("anthropic", state, headers, body).await
}
...
pub async fn handle_anthropic_count_tokens(
    ...
    handle_count_tokens("anthropic", state, headers, body).await
}
...
pub async fn handle_openai_chat(
    ...
    handle_proxy("openai", state, headers, body).await
}
...
pub async fn handle_openai_responses(
    ...
    handle_proxy("openai_responses", state, headers, body).await
}
...
async fn handle_proxy(
    ...
    let fwd_body = match (client_protocol, provider_format) {
        ...
        ("openai", ProviderFormat::Anthropic) => {
            ...
            // Note: OpenAI Chat requests don't have thinking blocks, no need to strip
            let mut b = body.clone();
            b["model"] = Value::String(route.model.clone());
            normalize_roles(&mut b);
            b
        ...
        ("openai_responses", ProviderFormat::Openai) => {
            // Responses client → OpenAI Chat provider: need to convert
            // For now, passthrough with model swap (Responses→Chat request
            // conversion is rare; the client would need to send Chat-formatted
            // input in Responses envelope)
            let mut b = body.clone();
            b["model"] = Value::String(route.model.clone());
            b
        }
        ("openai_responses", ProviderFormat::Anthropic) => {
            // Responses client → Anthropic provider: passthrough with model swap
            let mut b = body.clone();
            b["model"] = Value::String(route.model.clone());
            b
        }
    ...
    // Forward client headers (skip hop-by-hop and auth we already set)
        ...
        if lower == "anthropic-beta" {
            if let Ok(v) = value.to_str() {
                ...
                if !filtered.is_empty() {
                    req_builder =
                        req_builder.header("anthropic-beta", filtered.join(", "));
                }
                continue;
            }
        }
    ...
    if is_streaming {
        ...
        match (client_protocol, provider_format) {
            ("anthropic", ProviderFormat::Openai) => {
                ...
                let openai_resp: Value = serde_json::from_slice(&resp_body).unwrap_or_else(|e| { log::warn!("[Proxy] failed to parse upstream response as JSON: {}", e); Value::Null });
            ...
            ("anthropic", ProviderFormat::OpenaiResponses) => {
                ...
                let responses_resp: Value = serde_json::from_slice(&resp_body).unwrap_or_else(|e| { log::warn!("[Proxy] failed to parse upstream response as JSON: {}", e); Value::Null });
            ...
            _ => {
                ...
                let mut resp_value: Value = serde_json::from_slice(&resp_body).unwrap_or_else(|e| { log::warn!("[Proxy] failed to parse upstream response as JSON: {}", e); Value::Null });
    ...
    let response = match route.format {
        ProviderFormat::Openai => {
            let openai_resp: Value =
                serde_json::from_str(&resp_text).unwrap_or_else(|e| { log::warn!("[Test] failed to parse response as JSON: {}", e); Value::Null });
