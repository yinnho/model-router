...
All notable changes to Model Router will be documented in this file.
...
- **Claude Desktop Third-Party Provider Switching via Proxy Gateway**: Added a dedicated Claude Desktop surface that brokers third-party Claude providers through Model Router's in-app proxy, with a routing-support badge for providers that need it, role-based model route mapping locked to `sonnet` / `opus` / `haiku`, Copilot/Codex OAuth provider reuse, a redesigned Claude Code import flow, app-switcher differentiation between "Claude Code" and "Claude Desktop", and 44 provider presets translated from the Claude Code catalog.
...
- **Claude Desktop Operational Notes**: Switching a Claude Desktop provider now writes Model Router's managed 3P profile and requires restarting Claude Desktop to take effect; proxy-mode providers require Model Router Local Routing to stay running.
...
- **Model Router Brand Surface Unified to modelrouter.io**: All in-app and README references now point at modelrouter.io as the sole official website; the release notes template also surfaces modelrouter.io.
...
- **Release Notes Template**: Surfaces `modelrouter.io` in the release notes template.
- **Brand Surface**: Documented `modelrouter.io` as the sole official website across READMEs and in-app references.
...
- **Hermes Config Health Scanner**: Removed the in-app Hermes config health scanner, warning banner, `scan_hermes_config_health` command, `HermesHealthWarning` type, and `HermesWriteOutcome.warnings` payload. Model Router now keeps the Hermes surface focused on active provider display, provider switching defaults, memory editing, and launching the Hermes Web UI for deep configuration.
...
- **Codex OAuth Responses SSE Aggregation**: Non-streaming Anthropic clients now receive JSON even when the ChatGPT Codex upstream forces OpenAI Responses SSE; Model Router aggregates the upstream SSE events before running the non-streaming transform (#2235).
...
- **Hermes Memory Panel**: Added a Memory panel for editing `MEMORY.md` and `USER.md` directly from Model Router, with an enable switch, character-count limits, and a live save flow. Replaces the Prompts entry for Hermes.
...
- **Application-Level Window Controls**: Added an opt-in setting to render Model Router's own minimize / toggle-maximize / close buttons instead of the system decorations, materially improving the experience on Linux Wayland where compositor-drawn buttons can become inert.
...
- **OpenClaw Config Directory Override**: Added a settings option to point Model Router at a custom `openclaw.json` location.
- **Hermes Config Directory Override**: Added a settings option to point Model Router at a custom `~/.hermes/config.yaml` location, backed by data-driven dispatch.
...
- **Hermes Deep Config Delegation**: Deep YAML knobs are now delegated to the Hermes Web UI via a direct launch action, rather than duplicated in the Model Router form.
...
- **Hermes YAML Pollution & OAuth MCP Auth Drop**: Round-tripping through Model Router no longer drops OAuth MCP `auth` blocks or pollutes unrelated YAML keys; guard tests added via `tests/hermes_roundtrip.rs`.
...
- **Lightweight Mode**: Added a tray-only mode that destroys the main window and keeps Model Router running from the system tray, with the window recreated when users reopen it.
...
- **Skills Discovery & Batch Updates**: Added SHA-256 based skill update detection, per-skill and batch update actions, a storage-location toggle between Model Router and `~/.agents/skills`, and public `skills.sh` search integration.
...
- **OpenClaw Integration**: Full management support for OpenClaw as the fifth application in Model Router, including provider switching, configuration panels (Env / Tools / Agents Defaults), Workspace file management (HEARTBEAT / BOOTSTRAP / BOOT), daily memory files, and additive overlay mode
...
- **Windows Title** - Display "Model Router" instead of default "Tauri app" in window title
...
- **Import/Export** - Restrict SQL import to Model Router exported backups only; refresh providers immediately after import
...
- **Deep link support** - Import Gemini providers via `modelrouter://` protocol
...
#### Deep Link Protocol (modelrouter://)

- **Protocol registration** - `modelrouter://` URL scheme for one-click imports
