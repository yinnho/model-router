...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（Model Router の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
Model Router が設定を変更する際の優先順位：

1. **Model Router データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（Model Router がバックフィルする）
- Model Router の `settings.json`
...
Model Router v3.7.0 で JSON ファイルから SQLite に移行しました：
