...
| 字段 | 说明 |
|------|------|
| `models.providers` | 供应商配置（映射为 Model Router 的"供应商"） |
| `env` | 环境变量配置 |
...
Model Router 修改配置时的优先级：

1. **Model Router 数据库** - 单一事实源 (SSOT)
...
- CLI 工具的配置文件（会被 Model Router 回填）
- Model Router 的 `settings.json`
...
Model Router v3.7.0 从 JSON 文件迁移到 SQLite：
