...
Model Router for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/yinnho/model-router/releases).
...
2. Add Model Router to your antivirus software's whitelist
...
chmod +x Model-Router-*.AppImage

# If it still fails, try
./Model-Router-*.AppImage --no-sandbox
```
...
**Solutions**:
1. Confirm the file was exported by Model Router
...
**Windows**:
- Check taskbar settings to ensure the Model Router icon is not hidden
...
Toggle "Lightweight Mode" from the system tray menu. The main window closes, and Model Router runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
1. Visit [GitHub Issues](https://github.com/yinnho/model-router/issues)
