...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to Model Router's "providers") |
...
Model Router's priority when modifying configurations:

1. **Model Router Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by Model Router)
- Model Router's `settings.json`
...
Model Router v3.7.0 migrated from JSON files to SQLite:
