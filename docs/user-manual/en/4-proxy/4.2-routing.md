...
App routing means letting Model Router route a specific application's API requests through the local routing service.
...
When routing is enabled, Model Router modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, Model Router will:
...
Before enabling routing, Model Router backs up the original configuration and restores it when disabled.
