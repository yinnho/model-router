...
This section describes how to configure Model Router according to your preferences.
...
Model Router supports three languages:
...
When enabled, Model Router automatically runs when the system starts.
...
Starting from v3.13.0, Model Router adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger Model Router via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If Model Router runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, Model Router automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that Model Router uses when opening a terminal.
...
The storage location for Model Router's own data, defaulting to `~/.cc-switch/`.
...
Configure Model Router's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, Model Router automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
- Model Router automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, Model Router displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current Model Router version number, with support for:
