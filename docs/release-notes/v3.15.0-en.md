...
>
> Model Router is a **fully free and open-source** desktop app, and we **do not charge users any fees**. Multiple imposter websites have recently been spotted impersonating Model Router to solicit payments and harvest account credentials, with some users already reporting financial losses. Please only obtain the software through the official channels listed below:
>
> | Channel            | Only Official                                                                  |
> | ------------------ | ------------------------------------------------------------------------------ |
> | Website            | **[modelrouter.io](https://modelrouter.io)**                                         |
> | Source             | **[github.com/yinnho/model-router](https://github.com/yinnho/model-router)** |
> | Downloads          | **[GitHub Releases](https://github.com/yinnho/model-router/releases)**        |
> | Author             | **[@yinnho](https://github.com/farion1231)**                               |
> | Report an Imposter | **[GitHub Issues](https://github.com/yinnho/model-router/issues)**            |
>
> **Any "Model Router" website or client that asks you for payment, top-ups, or login credentials is fake.** If you have been tricked into paying, stop the transaction immediately and file a report through GitHub Issues so we can take down the imposter site as quickly as possible.
...
**[Use Model Router to configure, manage, and switch Claude Desktop providers in one place](../user-manual/en/2-providers/2.6-claude-desktop.md)**
...
Model Router now treats **Claude Desktop** as a first-class managed surface alongside Claude Code / Codex / Gemini / OpenCode / OpenClaw / Hermes.

- New dedicated Claude Desktop panel that brokers third-party providers to Claude Desktop through Model Router's in-app proxy gateway
...
ChatGPT Codex providers no longer rely on a hardcoded model selection — Model Router fetches a **live model list** from the ChatGPT backend on demand.
...
Switching a Claude Desktop provider writes Model Router's managed 3P profile and **requires restarting Claude Desktop** to take effect; proxy-mode providers require Model Router's Local Routing to stay running while in use.
...
### Model Router Brand Surface Unified to modelrouter.io

All in-app and README "official website" references now point at modelrouter.io as the sole official site; the release notes template also surfaces modelrouter.io.
...
The release notes template now surfaces `modelrouter.io`.

### Brand Surface

Documented `modelrouter.io` as the sole official website across READMEs and in-app references.
...
**Claude Desktop Third-Party Provider Switching via Proxy Gateway**: Routing Claude Desktop traffic through Model Router's in-app proxy gateway to a third-party provider exposes those requests to that provider's billing, compliance, and data-retention policies — read the target provider's terms of service before using.

By enabling these features, users **accept all associated risks**. Model Router is not responsible for any account restrictions, warnings, or service suspensions that result from using these features.

---

## Download & Installation

Visit [Releases](https://github.com/yinnho/model-router/releases/latest) to download the appropriate version.
...
| File                                     | Description                                           |
| ---------------------------------------- | ----------------------------------------------------- |
| `Model-Router-v3.15.0-Windows.msi`          | **Recommended** - MSI installer, supports auto-update |
| `Model-Router-v3.15.0-Windows-Portable.zip` | Portable, extract and run, no registry writes         |

### macOS

| File                             | Description                                             |
| -------------------------------- | ------------------------------------------------------- |
| `Model-Router-v3.15.0-macOS.dmg`    | **Recommended** - DMG installer, drag into Applications |
| `Model-Router-v3.15.0-macOS.zip`    | Extract and drag into Applications, Universal Binary    |
| `Model-Router-v3.15.0-macOS.tar.gz` | For Homebrew installation and auto-update               |

> macOS builds are Apple code-signed and notarized — install directly.

### Homebrew (macOS)

> 🎉 Model Router is now available in the official Homebrew cask repository — no need to add a custom tap!
...
>
> - `Model-Router-v3.15.0-Linux-x86_64.AppImage` / `.deb` / `.rpm`
> - `Model-Router-v3.15.0-Linux-arm64.AppImage` / `.deb` / `.rpm`

| Distribution                            | Recommended | Installation                                                           |
| --------------------------------------- | ----------- | ---------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`      | `sudo dpkg -i Model-Router-*.deb` or `sudo apt install ./Model-Router-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`      | `sudo rpm -i Model-Router-*.rpm` or `sudo dnf install ./Model-Router-*.rpm`  |
| openSUSE                                | `.rpm`      | `sudo zypper install ./Model-Router-*.rpm`                                |
| Arch Linux / Manjaro                    | `.AppImage` | Add execute permission and run, or use AUR                             |
| Other distros / not sure                | `.AppImage` | `chmod +x Model-Router-*.AppImage && ./Model-Router-*.AppImage`              |
